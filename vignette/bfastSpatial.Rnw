...
<<'getSceneinfo-tura', fig.show='hide'>>=
...
The number of available observations in a raster time series can be calculated by using \code{countObs()}. This function "drills" through pixels of a time series and counts the number of pixels with a non-NA value. Optionally, any other value can be supplied as a substitute for NA (e.g. the number of non-zero values per pixel can also be queried).
...
<<'countObs-asperc', fig.show='hide', results='hide'>>=
# valid observations
...
<<'summaryBrick-functions', fig.show='hide'>>=
...
<<'summaryBrick-narm', fig.show='hide'>>=
...
<<summaryBrick-custom, fig.show='hide'>>=
...
<<'annualSummary-ETM', fig.show='hold'>>=
...
<<'bfmPixel-targcell', fig.show='hide'>>=
targcell <- 3492
