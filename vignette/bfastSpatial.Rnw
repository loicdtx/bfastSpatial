...
\section{Downloading Landsat data from Earth Explorer/espa}
...
USGS is planning to support the delivery of data in different formats (HDF, GeoTiff, binary). GeoTiff is a common and easy to handle format of raster data and should be your preferred choice if you are working in Windows. A convenient way of placing an order at the moment is via \href{http://espa.cr.usgs.gov/}{espa} (you will need to request a username/password, which is different from your USGS credentials). Once you have entered the credentials provided to you, you may use the interface to upload the list of scenes you want to have pre-processed, select the layers of interest and place your order. A scene list is a simple text file with a scene ID (eg: LT52302701999134) on each line. Note that this order system is likely to evolve in the future.
...
Let's illustrate with an example how these functions work. We added subsets of Landsat data to the package in order to illustrate the pre-processing steps. The test datasets are stored under the \code{external/} directory of the installed package. To list the content of that directory, run the following comand.
<<'listPackageData'>>=
...
\begin{itemize}
    \item A mask (\code{mask=}) had to be selected; in that case \code{'fmask'}, which is one of the layers of the landsat archive file delivered by USGS/ESPA. For details about that mask and know which values to keep (\code{keep=}), you can visit \href{http://code.google.com/p/cfmask/}{this page}, or for general information on the layers provided in the archive, see the \href{http://landsat.usgs.gov/documents/cdr_sr_product_guide.pdf}{product guide}.
    \item The option \code{delete=} does not delete the original tarball/zip file, but instead the intermediary output, which is the layers uncompressed from the archive later used to compute the vegetation indices. This option is usefull in case you want to keep the data in their compressed form only, hence reducing disk space usage.
    \item No extent object (\code{e=}) was used in the example. However, it is advised to always use an extent to crop the layers produced. This way we ensure that all layers have exactly the same extent and can be used to create a multilayer object, which is the next step in the pre-processing chain. To obtain an extent, you may either use the interactive function \code{drawExtent()} to draw an extent manually, or use an existing spatial object (raster or vector).
    \item Masking and cropping are optional but highly recomended if you intend to perform subsequent time-series analysis with the layers produced.
    \item No output filename was required; that variable is automatically generated from the input file and the \code{vi} selected.
    \item The extension of the output is by default .grd. This is the native format of the raster package. This format has many advantages over more common GeoTiff or ENVI formats.
    \begin{itemize}
        \item Original layer names are kept in multi-layer objects.
        \item There is a possibility to add a z dimension to multilayer objects, useful for storing time information for instance.
        \item Can be read and written without requiring the \code{rgdal} package (which is not always easy to install).
    \end{itemize}
\end{itemize}

\subsection{Running \code{processLandsat} in batch mode}
Now that we have seen how to generate 
For that, use the \code{processLandsatBatch} function; it allows to perform the above step in batch mode over a list and supports parallel implementation for greater speed.\\
...
# Stack the layers
s <- timeStack(x=list, filename=stackName, datatype='INT2S')
...
\begin{itemize}
    \item It is absolutely necessary for this function to work that the input layers have the same extent. Two Landsat scenes belonging to the same path/row, but acquired at different times often have slightly different extents. We therefore recomend to always use an extent object in \code{processLandsat} and \code{processLandsatBatch}, even when working with full scenes.
    \item Time information is automatically extracted from the layer names (using \code{getSceneInfo}) and written to the z dimention of the stack.
    \item We chose to write the output to the .grd format, which allows the time information to be stored as well as the original layer names.
    \item The \code{x=} argument can also simply be the directory containing the layers; in which case we recomend using \code{pattern=} as well in order to ensure that only the desired files are included.
\end{itemize}
