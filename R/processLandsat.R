...
#' @param delete Logical. Should hdf files be deleted after vegetation index calculated? (usefull for disk space management; hdf files are very voluminous and a user may want to keep the Landsat archive in compressed format only)
#' @param ... Arguments to be passed to \link{hdf2ndvi}. Do not specify \code{filename} since it is automatically generated
...
processLandsat <- function(x, hdfdir, outdir, untar=TRUE, delete=FALSE, ...) {
    ...
    if(untar){
        untar(x, exdir=hdfdir, files='*.hdf', extras='--wildcards')
        x <- list.files(hdfdir, pattern=glob2rx(sprintf('lndsr.%s*.hdf', name)), full.names=TRUE)
    }
    name <- str_extract(string=basename(x), '(LT4|LT5|LE7)\\d{13}')   
    hdf2ndvi(x=x, filename=sprintf('%s/ndvi.%s.grd', outdir, name), datatype='INT2S', ...)
    if(delete) {
        file.remove(x)
    } 
}